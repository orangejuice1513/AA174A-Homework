...
class TrajectoryTracker:
    ...
    def compute_control(self, x: float, y: float, th: float, t: float) -> T.Tuple[float, float]:
        ...
        # Calculate the robot's current velocity components from the last command
        xd = self.V_prev * np.cos(th)
        yd = self.V_prev * np.sin(th)

        # Calculate the virtual controls (u1, u2) using the feedback law
        u1 = xdd_d + self.kpx * (x_d - x) + self.kdx * (xd_d - xd) 
        u2 = ydd_d + self.kpy * (y_d - y) + self.kdy * (yd_d - yd)

        # Convert virtual controls into real controls (a, om)
        a = u1 * np.cos(th) + u2 * np.sin(th)

        # Handle the singularity by ensuring the velocity in the denominator is not too small
        v_denom = max(self.V_prev, V_PREV_THRES)

        numerator = -u1 * np.sin(th) + u2 * np.cos(th)
        om = numerator / v_denom

        # Integrate acceleration 'a' to get the final velocity command 'V'
        V = self.V_prev + a * dt
